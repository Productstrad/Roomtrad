...
@Component
public class Trade_operatelogDao {

	private Logger logger = Logger.getLogger(Trade_operatelogDao.class);
	private String proxool_M="proxool.defaultDB";
	private String proxool_S="proxool.defaultDB_s0";	
	
	public int insert(Trade_operatelog vo){
		int result = 0;		
		if( null == vo ){
			return -1;
		}		
	 ...
	 * @return	 
	 */
	public List<Trade_operatelog> find(Map<String, Object> paramsMap,int pageNo,int pageSize){
		List<Trade_operatelog> list = null;
		try{
			...
			list=VOKit.rs2BeanList(rs, Trade_operatelog.class);
		}catch (Exception e) {
			logger.error("", e);
		} 
		return list;
	 ...
	 * @return	
	 */
	public Trade_operatelog findByPK(Object id) {
	 ...
	 * @return	 
	 */
	public int update(Trade_operatelog vo) {
		int result = 0;		
		if( vo.getId()==null){
			return -1;
		}		
