...
@Component
public class Trade_roleDao {

	private Logger logger = Logger.getLogger(Trade_roleDao.class);
	private String proxool_M="proxool.defaultDB";
	private String proxool_S="proxool.defaultDB_s0";	
	
	public int insert(Trade_role vo){
		int result = 0;		
		if( null == vo ){
			return -1;
		}		
	 ...
	 * @return	 
	 */
	public List<Trade_role> find(Map<String, Object> paramsMap,int pageNo,int pageSize){
		List<Trade_role> list = null;
		try{
			...
			list=VOKit.rs2BeanList(rs, Trade_role.class);
		}catch (Exception e) {
			logger.error("", e);
		} 
		return list;
	 ...
	 * @return	
	 */
	public Trade_role findByPK(Object id) {
	 ...
	 * @return	 
	 */
	public int update(Trade_role vo) {
		int result = 0;		
		if( vo.getId()==null){
			return -1;
		}		
