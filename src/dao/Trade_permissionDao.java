...
@Component
public class Trade_permissionDao {

	private Logger logger = Logger.getLogger(Trade_permissionDao.class);
	private String proxool_M="proxool.defaultDB";
	private String proxool_S="proxool.defaultDB_s0";	
	
	public int insert(Trade_permission vo){
		int result = 0;		
		if( null == vo ){
			return -1;
		}		
		try {
			SqlBuilder sql=new SqlBuilder("insert into trade_permission");			
	 ...
	 * @return	 
	 */
	public List<Trade_permission> find(Map<String, Object> paramsMap,int pageNo,int pageSize){
		List<Trade_permission> list = null;
		try{
	 ...
	 * @return	
	 */
	public Trade_permission findByPK(Object id) {
	 ...
	 * @return	 
	 */
	public int update(Trade_permission vo) {
		int result = 0;		
		if( vo.getId()==null){
			return -1;
		}		
		try {
			SqlBuilder sql=new SqlBuilder("update trade_permission");				
	...
	public int update(Map<String, Object> paramsMap) {
		...
		try {
			...
			sql.appendSetParam("trad_Permissioncol_set", "trad_Permissioncol=?");			
	...
	public int delete(int id) {
		...
		try {
			SqlBuilder sql=new SqlBuilder("update trade_permission set state=-1 WHERE id=?");
			//按顺序添加sql参数
			sql.params.add(id);			
			result = DBFactory.getDBObject(proxool_M).update(sql);					
		}catch (Exception e) {
